...
while en_cours:
    for evenement in pygame.event.get():
        if evenement.type == pygame.QUIT:
            ...
            elif ecran in ["regles", "puzzle", "gagne"]:
    ...
    if ecran == "menu":
        ...
        fenetre.blit(texte_r, (bouton_regles.x + (bouton_regles.width - texte_r.get_width()) // 2,
        ...
        fenetre.blit(texte_retour, (bouton_retour.x + (bouton_retour.width - texte_retour.get_width()) // 2,
        ...
        for i in range(6):
            if jeux_gagnes[i] == "0":
                x = puzzle_x + (i % 3) * carre_largeur
                y = puzzle_y + (i // 3) * carre_hauteur
                pygame.draw.rect(fenetre, ROUGE, (x, y, carre_largeur, carre_hauteur))

        if all(j == "1" for j in jeux_gagnes):
            ecran = "gagne"

    elif ecran == "gagne":
        fenetre.fill(BLANC)
        texte_gagne = pygame.font.SysFont(None, 80).render("Vous avez gagné !", True, BLEU)
        fenetre.blit(texte_gagne, ((LARGEUR - texte_gagne.get_width()) // 2,
                                   (HAUTEUR - texte_gagne.get_height()) // 2))
        pygame.draw.rect(fenetre, GRIS, bouton_retour, border_radius=10)
        texte_retour = police.render("Retour", True, BLEU)
        fenetre.blit(texte_retour, (bouton_retour.x + (bouton_retour.width - texte_retour.get_width()) // 2,
                                    bouton_retour.y + (bouton_retour.height - texte_retour.get_height()) // 2))
